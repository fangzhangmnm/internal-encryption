// @internal/encryption 容器验收（自 internal-store test/crypto-container.test.mjs 迁入 2026-08-28 立户）。
//
...
const { createEncryption, makeGuid, PEEK_TAIL_WINDOW, scanEncPeekFromEnd, decryptPeek, encryptPeek } = await import("../src/index.ts");
const { zipPack, zipUnpack } = await import("./fixtures/zip.ts");
const { pack7z, unpack7z } = await import("./fixtures/sevenzip.ts");
// codec 经工厂注入（模块级 configureCryptoCodec 单例已随立户退役——替身/单例大清洗 2026-08-27/28）。
const enc = createEncryption({ codec: { zipPack, zipUnpack, pack7z, unpack7z } });
const { packContainer, unpackContainer, looksEncryptedContainer } = enc;
