...
#
# 在**消费方仓根**运行：
#   bash "../20260828 internal-encryption/scripts/pull-package.sh"          # 收最新已发版
#   bash "../20260828 internal-encryption/scripts/pull-package.sh" 0.2.0    # 收指定版本
#
# 干的事：已发版 tgz 落 vendor-pkgs/ → package.json 的 @internal/encryption 指到新版（file:）
...
tgz="internal-encryption-$ver.tgz"
...
[ "$pv" = "@internal/encryption@$ver" ] || die "验货失败：tgz 里是 $pv，期望 @internal/encryption@$ver。"

# 消费方 package.json 指针（无 dependencies/条目则新建——首次接线也走这脚本）。
node -e '
  const fs = require("fs");
  const p = JSON.parse(fs.readFileSync("package.json", "utf8"));
  (p.dependencies ??= {})["@internal/encryption"] = "file:./vendor-pkgs/" + process.argv[1];
  fs.writeFileSync("package.json", JSON.stringify(p, null, 2) + "\n");
' "$tgz"

# 旧版本 tgz 清掉（历史在 git；vendor-pkgs 只留 pin 的这一份）。
find "$APP_DIR/vendor-pkgs" -maxdepth 1 -name 'internal-encryption-*.tgz' ! -name "$tgz" -delete

npm install --no-audit --no-fund

echo "[pull-package] ✓ @internal/encryption@$ver 已就位（来源：$src）"
